...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { if (!isSupabaseConfigured()) return; createClient().auth.getUser().then(({ data }) => setUserEmail(data.user?.email ?? "")); }, [pathname]);
  async function signOut() { await createClient().auth.signOut(); router.replace("/ingreso"); router.refresh(); }
  const userName = userEmail ? userEmail.split("@")[0] : "Gerencia"; const userInitial = userName.charAt(0).toUpperCase();
  ...
  return <div className={`app-shell ${collapsed ? "sidebar-is-collapsed" : ""}`}>
    ...
    <aside className={`sidebar ${open ? "sidebar-open" : ""} ${collapsed ? "sidebar-collapsed" : ""}`}>
      ...
      <div className="sidebar-user"><div className="avatar">{userInitial}</div><div><strong>{userName}</strong><span>{userEmail || "Gestión institucional"}</span></div>{userEmail && <button className="icon-button" aria-label="Cerrar sesión" title="Cerrar sesión" onClick={signOut}><LogOut size={16}/></button>}</div>
    </aside>
