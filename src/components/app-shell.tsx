...
import { BarChart3, BookOpen, FileText, FolderKanban, GitBranch, Home, LogOut, Menu, PanelLeftClose, PanelLeftOpen, Search, Settings, ShieldCheck, X } from "lucide-react";
...
const nav = [
  ...
  { href: "/revisiones", label: "Revisiones", icon: ShieldCheck }, { href: "/indicadores", label: "Indicadores", icon: BarChart3 },
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className={`app-shell ${collapsed ? "sidebar-is-collapsed" : ""}`}>
    ...
    <aside className={`sidebar ${open ? "sidebar-open" : ""} ${collapsed ? "sidebar-collapsed" : ""}`}>
      ...
      <nav aria-label="Navegación principal"><p className="nav-label">GESTIÓN</p>{nav.map((item) => { const active = item.href === "/" ? pathname === "/" : pathname.startsWith(item.href) || item.aliases?.some((alias) => pathname.startsWith(alias)); const Icon = item.icon; return <Link key={item.href} href={item.href} className={`nav-item ${active ? "active" : ""}`} title={collapsed ? item.label : undefined} onClick={() => setOpen(false)}><Icon size={19}/><span>{item.label}</span></Link>; })}{user?.isSuperadmin && <><p className="nav-label nav-label-admin">SISTEMA</p><Link href="/administracion" className={`nav-item ${pathname.startsWith("/administracion") ? "active" : ""}`} title={collapsed ? "Administración" : undefined} onClick={() => setOpen(false)}><Settings size={19}/><span>Administración</span></Link></>}</nav>
    ...
    <div className="main-column"><header className="topbar"><button className="icon-button menu-button" aria-label="Abrir menú" onClick={() => setOpen(true)}><Menu size={21}/></button><Link href="/biblioteca" className="global-search" aria-label="Buscar en la biblioteca de procesos"><Search size={18}/><span>Buscar por código, proceso, área o responsable…</span><kbd>⌘ K</kbd></Link><div className="topbar-actions"><div className="topbar-avatar">{userInitial}</div></div></header><main className="page-wrap">{children}</main></div>
  </div>;
}
